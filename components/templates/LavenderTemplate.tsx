...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <section className="mb-5">
    <div className="flex items-center gap-3 mb-2.5">
      ...
      <h2 className="text-[10px] font-bold tracking-[0.18em] uppercase text-purple-400 whitespace-nowrap">{title}</h2>
...
export default function LavenderTemplate({ resume }: Props) {
  ...
  return (
    <div className="bg-white font-sans text-slate-800" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {/* Header */}
      <div className="px-9 pt-9 pb-6" style={{ background: "linear-gradient(135deg, #faf5ff 0%, #f3e8ff 100%)" }}>
        <h1 className="text-[26px] font-bold tracking-tight text-purple-900 leading-tight">{personalInfo.fullName}</h1>
        <p className="text-[11.5px] font-semibold text-purple-500 mt-1 tracking-wide">{personalInfo.jobTitle}</p>
        <div className="flex flex-wrap gap-x-5 gap-y-1 mt-3 text-[10.5px] text-purple-400">
      ...
      <div className="h-1 bg-gradient-to-r from-violet-300 via-purple-300 to-violet-200" />
                ...
                <div key={job.id}>
                  <div className="flex justify-between items-baseline">
                    <div>
                      ...
                      <span className="text-[11.5px] text-purple-500 font-medium ml-2">· {job.company}</span>
                    </div>
                  ...
                  <ul className="mt-1.5 space-y-1">
                    {job.bullets.map((b, i) => (
                      <li key={i} className="text-[11.5px] text-slate-600 leading-[1.6] flex gap-2">
                        <span className="text-purple-300 shrink-0 mt-[3px] text-[8px]">●</span><span>{b}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div key={p.id}>
                  <div className="flex justify-between items-baseline">
                    <div>
                      ...
                      <span className="text-[10.5px] text-purple-400 ml-2">{p.techStack}</span>
                    </div>
                  ...
                  {p.link && <p className="text-[10.5px] text-purple-400 mt-0.5">{p.link}</p>}
                ...
                <div key={r.id}>
                  <div className="flex justify-between items-baseline">
                    <div>
                      ...
                      <span className="text-[11.5px] text-purple-500 ml-2">· {r.role}, {r.institution}</span>
                    </div>
              ...
              {presentations.map((p) => (
                <div key={p.id} className="flex justify-between items-baseline">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-500 ml-2">· {p.event} <span className="capitalize text-purple-400">({p.type})</span></span>
                  </div>
        ...
        {skills.length > 0 && (
          <Section title="Skills">
            <div className="flex flex-wrap gap-1.5">
              {skills.map((s) => (
                <span key={s} className="text-[11px] font-medium text-purple-600 bg-purple-50 border border-purple-100 px-2.5 py-0.5 rounded-full">{s}</span>
              ))}
            </div>
          </Section>
        )}
