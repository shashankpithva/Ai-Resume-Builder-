...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <section className="mb-5">
    <div className="flex items-center gap-2.5 mb-2.5">
      <div className="w-3 h-3 rounded-sm bg-violet-600 shrink-0" />
      <h2 className="text-[10px] font-bold tracking-[0.18em] uppercase text-violet-700">{title}</h2>
...
export default function BoldTemplate({ resume }: Props) {
  ...
  return (
    <div className="bg-white font-sans text-slate-800" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {/* Header */}
      <div className="bg-violet-700 px-9 pt-8 pb-5">
        <h1 className="text-[26px] font-black text-white tracking-tight leading-tight">{personalInfo.fullName}</h1>
        <p className="text-[11px] font-semibold text-violet-200 mt-1.5 uppercase tracking-[0.15em]">{personalInfo.jobTitle}</p>
      </div>
      <div className="bg-violet-900 px-9 py-2.5">
        <div className="flex flex-wrap gap-x-5 gap-y-1 text-[10.5px] text-violet-200">
              ...
              {workExperience.map((job) => (
                <div key={job.id}>
                  <div className="flex justify-between items-baseline">
                    <div>
                      <span className="text-[13px] font-bold text-slate-900">{job.title}</span>
                      <span className="text-[11.5px] text-violet-600 font-medium ml-2">· {job.company}</span>
                    </div>
                  ...
                  <ul className="mt-1.5 space-y-1">
                    {job.bullets.map((b, i) => (
                      <li key={i} className="text-[11.5px] text-slate-600 leading-[1.6] flex gap-2">
                        <span className="text-violet-400 shrink-0 font-bold mt-[1px] text-[9px]">◆</span><span>{b}</span>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="space-y-3">
              {projects.map((p) => (
                <div key={p.id}>
                  ...
                  {p.link && <p className="text-[10.5px] text-violet-500 mt-0.5">{p.link}</p>}
            ...
            <div className="space-y-3">
              {researchProjects.map((r) => (
                <div key={r.id}>
                  <div className="flex justify-between items-baseline">
                    <div>
                      <span className="text-[13px] font-bold text-slate-900">{r.title}</span>
                      <span className="text-[11.5px] text-violet-600 ml-2">· {r.role}, {r.institution}</span>
                    </div>
              ...
              {presentations.map((p) => (
                <div key={p.id} className="flex justify-between items-baseline">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-500 ml-2">· {p.event} <span className="capitalize text-violet-500">({p.type})</span></span>
                  </div>
            ...
            <div className="space-y-2">
              {education.map((e) => (
                <div key={e.id} className="flex justify-between items-baseline">
                  <div>
                    <span className="text-[13px] font-bold text-slate-900">{e.school}</span>
        ...
        {skills.length > 0 && (
          <Section title="Skills">
            <div className="flex flex-wrap gap-1.5">
              {skills.map((s) => (
                <span key={s} className="text-[11px] font-medium text-violet-700 bg-violet-50 border border-violet-100 px-2.5 py-0.5 rounded">{s}</span>
              ))}
            </div>
          </Section>
        )}
